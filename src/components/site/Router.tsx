      ...
      <div className="mx-auto max-w-[1400px] px-4 sm:px-6 py-24">
        ...
        <div className="mt-4 grid gap-10 lg:grid-cols-[1fr,auto] lg:items-end">
          ...
          <p className="max-w-md text-sm leading-relaxed text-muted-foreground mt-4">
            Arc testnet only for v1. Send or demo USDC, EURC, or cirBTC payments while stable FX API keys are pending.
          </p>
        </div>
      </div>
...
function SwapPanel() {
  ...
  const [txStatus, setTxStatus] = useState<TxStatus | "idle">("idle" as TxStatus | "idle");
...
const minReceived = estOut * (1 - slippageBps / 10_000);
  return (
    <div className="border border-border bg-background p-6">
      ...
      <div className="mt-5 border border-border bg-surface-1 p-4">
        ...
        <div className="mt-2 flex items-center justify-between gap-3">
          ...
          <CurrencySelect
            ...
            options={isPaymentMode ? ["USDC", "EURC", "cirBTC"] : STABLES.map((s) => s.symbol)}
          />
        </div>
      ...
      {/* You receive - hidden in Payment (MVP) mode to simplify the table */}
      {!isPaymentMode && (
      ...
      {/* Slippage (hidden in Payment/MVP mode) */}
      {!isPaymentMode && (
        <div className="mt-4 border border-border bg-surface-1 p-4">
        <div className="flex items-center justify-between text-mono-label">
      ...
      <button
        ...
        disabled={
          isPaymentMode
            ? !isConnected || !recipientAddress || !validateWalletAddress(recipientAddress) || !amount || paymentQuote.loading
